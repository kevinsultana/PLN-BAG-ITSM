...
export default function TicketTypeTable({
  ...
  }, [ticketTypes, orderBy, order, searchTerm]);

  const paginatedTypes = useMemo(() => {
    const start = (page - 1) * rowsPerPage;
    return sortedAndFilteredTypes.slice(start, start + rowsPerPage);
  }, [sortedAndFilteredTypes, page, rowsPerPage]);

  const totalPages = Math.ceil(sortedAndFilteredTypes.length / rowsPerPage);
