...
export default function SlaPolicyTable({
  ...
  }, [slaPolicies, orderBy, order, searchTerm]);

  const paginatedPolicies = useMemo(() => {
    const start = (page - 1) * rowsPerPage;
    return sortedAndFilteredPolicies.slice(start, start + rowsPerPage);
  }, [sortedAndFilteredPolicies, page, rowsPerPage]);

  const totalPages = Math.ceil(sortedAndFilteredPolicies.length / rowsPerPage);
